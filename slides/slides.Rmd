...
normal `r f2(nt$u_all)` and the two groups&rsquo; wages move together.
</div>
...
2. **TFP and distribution are separate questions.** $\psi$ moves the labor share
   while measured TFP barely changes.
...
| $\bar f_o \bar U_o = \bar H_o$, for $o = C$ | 1 | **step 2**, bisected at `steady.py:61` |
...
| mean filling $=$ mean $\bar\pi$ | 1 | **step 3**, bisected at `steady.py:81` |
]
...
The residual runs from $-\infty$ at $\bar U_C \to 0$ to $+0.13$ at $\bar U_C = \bar U$, so bisect on
...
row is the only *condition*, and it is the one bisected (`fill_resid`, `steady.py:75`,
bisected at `:81`). How wrong a guess is = the mean filling rate it implies, minus the
...
$t_0$ and deliberately outside the 44: `steady.py:48`.
]
]
