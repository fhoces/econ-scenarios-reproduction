...
4. <b>Gaps and the AI-sensitive wage.</b> Compare where each group's employment is with where
   the targets say it should be: the <b>overhang</b> (<i>G</i><sub>o,t</sub>, workers above the
   target) and the <b>shortfall</b> (<i>B</i><sub>o,t</sub>, workers below it), plus the
   <b>attached force</b> (<i>N</i><sub>C,t</sub>, the employed and the extra unemployed who still
   count as belonging to the group). Then move the AI-sensitive wage
   (<i>w</i><sub>C,t</sub>/<i>w</i><sub>t</sub>) a monthly fraction 1&minus;&xi;<sub>m</sub> of the
   way toward the wage that would clear it (<i>w</i><sup>c</sup><sub>C,t</sub>), where
   &xi;<sub>m</sub> = &xi;<sup>1/12</sup> is the yearly stickiness &xi; on a monthly clock.
...
.blue-box[
The reproduction: `aiscen` Python package, 128 tests, reproduces all 169 cells of
