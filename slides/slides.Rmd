...
model, which reproduces the paper's Tables 3, 5 and 6 to the printed precision, except
two documented rounding-fork cells. The deck reads that package's CSV output.
]
...
.highlight-box[
One genuine discrepancy: the all-workers unemployment rate is 0.07 points low in the
substantial scenario (modest and extreme are off by 0.04 points or less, and No-AI
matches exactly), because Table 1 rounds the normal pool to 0.038 while the paper's own
...
.blue-box[
The reproduction: `aiscen` Python package, 102 tests, reproduces Tables 3, 5 and 6 to
the printed precision except two documented rounding-fork cells. Narrative walkthrough
with every equation in `repro.qmd`.
]
...
and extreme are off by 0.04 points or less, and No-AI matches exactly). At
