...
.figfull h1 { font-size: 27px; margin: 0 0 7px; }
...
[overview](../index.html) &middot; [the report](../repro.html) &middot;
[the explorer](../explorer/) &middot;
...
<tr><td style="padding:6px;"><b>1. The claim</b></td><td style="padding:6px;">What the paper asserts, and the seven inputs it rests on</td><td style="text-align:center;">3&ndash;17</td></tr>
<tr><td style="padding:6px;"><b>2. The model</b></td><td style="padding:6px;">Task-based production, the assignment rule, prices and shares, TFP, the labor share, employment, ideas, unemployment: where every equation comes from</td><td style="text-align:center;">18&ndash;33</td></tr>
<tr><td style="padding:6px;"><b>3. Solving the model for US inputs</b></td><td style="padding:6px;">Calibration, the steady state, all 44 equations and the order they run in, the two root-finds</td><td style="text-align:center;">34&ndash;47</td></tr>
<tr><td style="padding:6px;"><b>4. Results: some scenarios and the explorer</b></td><td style="padding:6px;">The three scenarios one at a time, robustness, the survey, what reproduces, and the whole grid</td><td style="text-align:center;">48&ndash;60</td></tr>
<tr><td style="padding:6px;"><b>Backup</b></td><td style="padding:6px;">Derivations, month 0 step by step, the gain <i>a</i>, the slop extension</td><td style="text-align:center;">61&ndash;80</td></tr>
</table>
...
t3 |>
  ...
  transmute(Outcome = relabel(str_trim(row)),
            `No AI` = pub_no_ai, Modest = pub_modest,
            Substantial = pub_substantial, Extreme = pub_extreme) |>
...
**3.02 to 3.09**, while the labor share moves **59.8 to 54.9** and AI-sensitive employment
...
.highlight-box[
.small[
**Status:** a range anchored to usage data, a point value assumed. Moving it across its range shifts the labor share from 57.3 to 55.4 percent while TFP moves only 3.06 to 3.09.
]
]
...
.highlight-box[
.small[
**Status:** the **least identified** of the seven, and the second-strongest of the four frictions: moved alone it shifts 2030 AI-sensitive unemployment by 1.35 points across the paper's range, behind search efficiency $\mu$ at 1.47 and ahead of $\psi$ at 0.75. More if you allow full offset.
]
]
...
.highlight-box[
.small[
**Status:** baseline measured, scenario values assumed. It is the **weakest of the four frictions**: moved alone it shifts 2030 AI-sensitive unemployment by 0.66 points. Of all seven inputs only the gain $a$ moves it less, by 0.24.
]
]
...
demand falls short of the payroll; the AI-sensitive wage moves a fraction
1&minus;<span class="sym">&xi;</span> of the way to its clearing level each month.<br>
<b>Capital answers prices, not a plan.</b> Supply is a schedule with elasticity
<span class="sym">&epsilon;</span>.<br>
<b>Ideas accumulate.</b> A share of GDP goes to research, with fishing-out
<span class="sym">1&minus;&phi;<sub>R</sub></span>.
...
<div class="ser">&#9881;&#65039; measured TFP</div>
...
<div class="eqtag">the three channels inside <b>(14)</b></div>

$$\Delta \ln s_{L,t} \approx -\underbrace{(1-\rho)\psi_t m_t d_t}_{\text{displacement}} + \underbrace{(1-\sigma)\psi_t m_t d_t a_t}_{\text{weak link}} - \underbrace{(1-\sigma)\tfrac{s_{K,t0}}{s_{L,t0}}\Delta \ln r_t}_{\text{dearer capital}} \;\; \text{(14), to first order}$$
...
$$\frac{\ell_{C,t0} - \ell^*_{C,t}}{\ell_{C,t0}} = \frac{s_{N,t0}}{s_{C,t0}} \cdot \frac{\ell^*_{N,t} - \ell_{N,t0}}{\ell_{N,t0}}, \qquad \frac{s_N}{s_C} = 0.60 \;\; \text{(--)}$$
...
split 0.69&times; / 1.52&times; (55% responsive), <span class="sym">&mu;&#772;</span> = 0.17,
...
<span class="sym">q&#772;<sub>o</sub></span> and <span class="sym">f&#772;<sub>o</sub></span>
...
<img src="img/seq29.png" alt="The month as a sequence: AI inputs to targets by Proposition 1, targets to realized employment by the flows, realized employment to prices by system 39" style="display:block; width:100%; max-width:1210px; margin:4px auto 8px;">
...
# Result: Eleven Series (Four Plotted Here) For Three Scenarios. <a href="../explorer/" class="reflink" target="_blank">Explore All Scenarios Here</a>
...
<div class="ser" style="flex:1; display:flex; align-items:center; border-left:3px solid #e2e7ec; padding-left:7px; margin:0; color:#9aa6b2;">&#9881;&#65039;&nbsp; measured TFP</div>
...
.tiny[
Only the four connected series are plotted here; the rest are in the tables. Four of
those seven (TFP, average wage, labor share, capital stock) also appear in the
explorer; three (employment by group, occupation switching, the ideas stock) are
tables-only.
]
...
<div class="bul">&bull;&nbsp; AI ends up able to do <b>a fifth</b> of today&rsquo;s cognitive work <span style="color:#5b6873;">(<i>m</i> = `r f2(iv("modest","m"))`)</span></div>
...
<div class="em">&#9881;&#65039; measured TFP <b>`r f1(ov("modest","tfp"))`%</b></div>
...
<span class="note">Normal times: unemployment 2.91 and 5.36 by group, 3.84 overall,
...
`r f1(ov("modest","capital"))` percent up against a TFP gain of `r f1(ov("modest","tfp"))`
(&epsilon; = 3).<br><br>
<b>The frictions never bind.</b> Displaced workers move between occupations as easily as they do
today (&mu; at its normal-times 0.17), so unemployment ends `r f2(ov("modest","u_all"))` against a
normal 3.84 and the two groups&rsquo; wages move together.
</div>
...
<div style="flex:0.94;">
<h4>If you believe that&hellip;</h4>
<div class="bul">&bull;&nbsp; AI ends up able to do <b>a third</b> of today&rsquo;s cognitive work <span style="color:#5b6873;">(<i>m</i> = `r f2(iv("substantial","m"))`)</span></div>
...
<div class="em">&#9881;&#65039; measured TFP <b>`r f1(ov("substantial","tfp"))`%</b></div>
...
worker, and only a quarter of displaced tasks come back (&psi; = 0.75, &rho; = 0.25), so the labor
share falls 60.0 to
`r f1(ov("substantial","lab_share"))`.<br><br>
<b>Complementarity does the distributing.</b> Because tasks are complements (&sigma; = 0.5),
...
sits at `r f1(ov("substantial","wage_C"))`.<br><br>
<b>Reallocation is slow, but still feasible.</b> Crossing occupations is half as easy as normal
(&mu; = 0.08), which lifts AI-sensitive unemployment only to
...
<div style="flex:0.94;">
<h4>If you believe that&hellip;</h4>
<div class="bul">&bull;&nbsp; AI ends up able to do <b>half</b> of today&rsquo;s cognitive work <span style="color:#5b6873;">(<i>m</i> = `r f2(iv("extreme","m"))`)</span></div>
...
<div class="em">&#9881;&#65039; measured TFP <b>`r f1(ov("extreme","tfp"))`%</b></div>
...
all comes back as new labor tasks (&psi; = 0.90, &rho; = 0), so virtually the whole wage bill of
every automated task moves to capital: the labor share falls 60.0 to
`r f1(ov("extreme","lab_share"))` even as GDP rises `r f1(ov("extreme","gdp"))` percent.<br><br>
<b>The frictions turn reallocation into rationing.</b> Crossing occupations is four times harder
than normal and the wage falls only slowly (&mu; = 0.04, &xi; = 0.5), so firms want fewer
...
`r f1(ov("extreme","u_N"))` percent while its wage rises `r f1(ov("extreme","wage_N"))`: the same
...
tibble(
  ...
  Cells = c(80, 40, 49),
  `Largest deviation` = c(0.08, 0.05, 0.11)
) |>
  knitr::kable(format = "html")
```
...
.highlight-box[
Two cells need the tolerance widened, both unemployment rates and both from one
rounding: Table 1 puts the normal pool at 0.038 while the paper's own derivation uses
0.0384. The all-workers rate is 0.07 points low in the substantial scenario, and the
AI-sensitive rate 0.08 points low in the No-AI column. At 0.0384 the all-workers row
lands exactly; the No-AI rate improves to 2.85 and still prints as 2.8 against 2.9.
]
...
.blue-box[
The reproduction: `aiscen` Python package, 113 tests, reproduces all 169 cells of
Tables 3, 5 and 6 within tolerance, 153 of them to the printed digit. Narrative
walkthrough with every equation in `repro.qmd`.
]
...
The guess starts at **0**, the no-AI path, and walks out in steps of 0.02 until the residual
changes sign. The two $K$ columns are one stock reckoned twice: what firms want at that rental
rate, falling in it, against what savers offer, $\varepsilon \Delta \ln r$, rising in it. All
columns but the first two are in $10^{-3}$.

| trial $\Delta \ln r$ | | $s_{L}$ (14) | $\Delta \ln w$ (16) | $\Delta \ln (Y/L)$ (5) | $K$ firms want (17) | $K$ savers offer (6) | residual (18) |
|---|---|---|---|---|---|---|---|
...
labor demand, and this one. Same routine, different economy: here the price index is the residual
and employment is fixed. Columns other than the trial and $s_L$ are in $10^{-3}$.
...
Then, with no root-find left: $\tilde \ell_{C,t} = -0.000752$ (19), $u^x = 0$, $X = 0.000461$,
$G = 0.000482$, $\Delta \ln R = \Delta \ln Y = 0.001260$ (22),
...
# Backup: The Two Cells That Do Not Reproduce
...
points below the published figure in the substantial scenario (4.53 against 4.6; modest
and extreme are off by 0.04 points or less, and No-AI matches exactly), and an
AI-sensitive rate 0.08 points below it in the No-AI column (2.82 against 2.9; the other
three columns are off by 0.01 points or less). These two are the only cells in Tables 3,
5 and 6 that need the test tolerance widened.

At $\bar U = 0.0384$ the all-workers row lands exactly, six further cells that had
rounded to a different last digit fall into line, and nothing else moves. The AI-sensitive
rate in the No-AI column is the near miss: it improves to 2.85, which still prints as 2.8
rather than the paper's 2.9, so the pool accounts for the size of that gap without
closing it.
