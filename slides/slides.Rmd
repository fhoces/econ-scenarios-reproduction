...
.eqwall .eqnote { font-size: 15px; color: #333; margin: 0 0 4px; }
...
<tr><td style="padding:6px;"><b>1. The claim</b></td><td style="padding:6px;">What the paper asserts, and the seven inputs it rests on</td><td style="text-align:center;">3&ndash;16</td></tr>
<tr><td style="padding:6px;"><b>2. How the model is solved</b></td><td style="padding:6px;">Where the inputs come from, the steady state, all 44 equations and the order they run in</td><td style="text-align:center;">17&ndash;30</td></tr>
<tr><td style="padding:6px;"><b>3. Where the equations come from</b></td><td style="padding:6px;">Task-based production, prices, TFP, the labor share, ideas, unemployment</td><td style="text-align:center;">31&ndash;45</td></tr>
<tr><td style="padding:6px;"><b>4. Results and trust</b></td><td style="padding:6px;">Scenario by scenario, robustness, the survey, what reproduces</td><td style="text-align:center;">46&ndash;57</td></tr>
<tr><td style="padding:6px;"><b>Backup</b></td><td style="padding:6px;">Derivations, month 0 step by step, the gain <i>a</i>, the slop extension</td><td style="text-align:center;">58&ndash;77</td></tr>
</table>

.blue-box[
Every number in this deck comes from an independent Python reimplementation of the
model, which reproduces the paper's Tables 3, 5 and 6 to the printed precision. The
deck reads that package's CSV output.
]
...
inaccurate and demeaning to the work it excludes: it implies the nurse, the electrician and
the warehouse planner are not thinking, which is false and is not what the model claims. From
...
t3 |>
  ...
  transmute(Outcome = relabel(str_trim(row)),
...
separately in robustness over $\{1, 3, 6, \infty\}$ and $\{0, 0.5, 0.9\}$.

.highlight-box[
.small[
**Each of the seven is an empirical quantity with a true value**, and for each one you can say
what data would pin it down. The next seven slides take them one at a time. The explorer linked
below is a private artifact, shared per account.
]
]

<a href="https://claude.ai/artifact/6mNth93dAtpa5T4bisoRUu" class="nav-btn-br" target="_blank">the explorer &rarr;</a>
...
.pull-right[
**What is measured today.** The 0.5–0.9 range is taken from the Anthropic Economic Index automation-versus-augmentation split of observed usage. Of the four disruptiveness inputs, this has the best contemporaneous data.

**What would settle it.** Classifying observed AI interactions by whether a human stays in the loop, weighted to the economy rather than to the users of one product. Survey median **0.47**, the modest scenario's value.
]

.small[
**There is no separate augmentation input.** Augmentation is $1 - \psi$, and it delivers the
*same* cost saving $a$: the paper applies the saving whether a worker performs the instance
with AI or capital performs it outright (p. 11). So asking "what if AI mostly helps people
rather than replacing them" means lowering $\psi$, not raising some other input. Across the
full range from pure augmentation to pure automation, measured TFP moves only
**3.03 to 3.11**, while the labor share moves **59.8 to 54.9** and AI-sensitive employment
**−1.2% to −4.9%**. GDP does rise over that range (4.5 to 9.5), but through capital
deepening, not productivity.
]

.highlight-box[
.small[
**Status:** a range anchored to usage data, a point value assumed. Moving it across its range shifts the labor share from 57.3 to 55.4 percent while TFP moves only 3.07 to 3.10.
]
]
...
.pull-right[
**What is measured today.** The 0–0.5 range comes from the Acemoglu–Restrepo (2019) decomposition, which is **historical and not about AI**. It was not asked in the survey.
...
.highlight-box[
.small[
**Status:** the **least identified** of the seven, and it does more work than any other friction: moved alone it shifts 2030 AI-sensitive unemployment by 1.35 points across the paper's range, more if you allow full offset.
]
]
...
.highlight-box[
.small[
**Status:** baseline measured, scenario values assumed. It is the **weakest** of the seven: moved alone it shifts 2030 AI-sensitive unemployment by 0.66 points.
]
]
...
Everything so far treats the model as a box: seven numbers in, four paths out. That is already
enough to put your own view of AI on the same axes as the paper's three, which is exactly what
the explorer does.

<a href="https://claude.ai/artifact/6mNth93dAtpa5T4bisoRUu" class="inline-btn" target="_blank">set the inputs yourself &rarr;</a>
]

.pull-right[
**Or open the box.**

Parts 2 and 3 are the mechanism: how tasks become output, why the labor share moves without
productivity moving, where unemployment comes from, how the model is actually solved, and
which assumptions are carrying the results.
]
...
# Part 2: How the Model Is Solved
...
<div style="flex:1.85; border:1px solid #d6dbe0; padding:10px 13px 11px; background:#fbfcfd; font-size:13px; line-height:1.4; border-left:4px solid #2a78d6;">
<div style="font-weight:600; font-size:14.5px; margin-bottom:5px;">Inputs</div>
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">Seven that vary by scenario &mdash; their 2030 values</div>
<i>m</i> reach &middot; <i>d</i> adoption &middot; <i>a</i> gain &middot; &psi; automation &middot;
&rho; reinstatement &middot; &mu; search &middot; &theta;<sub>H</sub> posting
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">Given, but part of those seven's paths</div>
mid-2026 anchors <i>m</i> = 0.14 and <i>d</i> = 0.10, shared by all three; <i>a</i>'s anchor
differs (0.30 / 0.35 / 0.45); ceiling <i>d&#772;</i> = 1
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">Fixed across scenarios, varied in robustness</div>
&epsilon; = 3 capital supply &middot; &xi; = 0.5 wage rigidity
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">Calibrated constants, never varied</div>
<span style="color:#5b6873;">&sigma; = 0.5, s<sub>L</sub> = 0.60, s<sub>C</sub>/s<sub>L</sub> = 0.624,
r&#772; = 11.5%, &delta; = 5% &middot; ideas: 1&minus;&phi;<sub>R</sub> = 2.86, &lambda; = 1,
g = 1.67%, n = 0.33%, &iota;<sub>R</sub> = 3.5% rising 2.8%/yr &middot; labor market:
U&#772; = 3.8%, q&#772; = 0.11/yr split 0.69 / 1.52 (55% responsive), &mu;&#772; = 0.17,
&iota; = 1.27, mean &pi;&#772; = 0.65 &middot; dates 2024, 2026.5, 2030; step h = 1/12</span>
</div>

<div style="align-self:center; font-size:26px; color:#9aa6b2; padding:0 1px; flex:0 0 auto;">&rarr;</div>

<div style="flex:1; border:1px solid #d6dbe0; padding:10px 13px 11px; background:#fbfcfd; font-size:13px; line-height:1.4; border-left:4px solid #eb6834;">
<div style="font-weight:600; font-size:14.5px; margin-bottom:5px;">The model</div>
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">First, once</div>
The steady state, Eq. (38): solve for 9 SS values. &chi;, &pi;&#772;<sub>o</sub>, q&#772;<sub>o</sub> and
f&#772;<sub>o</sub> become constants; U&#772;<sub>o</sub> are the starting pools.
<div style="font-weight:600; font-size:11.5px; letter-spacing:.03em; text-transform:uppercase; color:#5b6873; margin-top:5px;">Then 73 months, 2024 to 2030</div>
The 44 equations, solved <b>sequentially</b> rather than all at once:<br>
<b>A</b> ideas (3) &middot; <b>B</b> capital, wage, shares, targets (9)<br>
<b>C</b> flows (23) &middot; <b>D</b> reporting (9)<br>
<b>Each month solves</b> employment &#8467;<sub>C</sub>, &#8467;<sub>N</sub>, pools
U<sub>C</sub>, U<sub>N</sub>, job-finding rates f<sub>C</sub>, f<sub>N</sub> by worker origin,
ideas &Delta;ln A, relative wage w<sub>C</sub>/w; they become next month's inputs.<br>
<span style="color:#5b6873;">The ordering is recursive, so only two rows need a root-find:
the rental rate, and the actual economy at realized employment.</span>
</div>

<div style="align-self:center; font-size:26px; color:#9aa6b2; padding:0 1px; flex:0 0 auto;">&rarr;</div>

<div style="flex:1; border:1px solid #d6dbe0; padding:10px 13px 11px; background:#fbfcfd; font-size:13px; line-height:1.4; border-left:4px solid #1baf7a;">
<div style="font-weight:600; font-size:14.5px; margin-bottom:5px;">Outputs: series</div>
GDP and measured TFP<br>
Wages: average, AI-sensitive, all other<br>
Labor share and the capital stock<br>
Employment and unemployment, by group<br>
<span style="color:#5b6873;">Every one a monthly gap against the same economy without AI.</span>
</div>
...
.blue-box[
.small[
The middle box comes in **two passes**. First **how the model is solved**, the rest of this part:
where the inputs come from, all 44 equations, the order they run in, and what is already known
when each one fires. Then **how it is derived**, Part 3: where each equation comes from and which
assumptions are carrying the results. Part 4 is what the right box says.
]
]
...
| $\psi$ range | 0.5–0.9 | **range** for input $\psi$ | Anthropic Economic Index automation-vs-augmentation split |
| $\rho$ range | 0–0.5 | **range** for input $\rho$ | Acemoglu–Restrepo (2019) decomposition |
]
...
are **assumptions**, not estimates; everything on this slide is anchored to data.
]
...
.pull-right[
.tiny[
<span style="color:#FF4500"><b>What comes out.</b></span> Nine objects, solved once, then held fixed for all 72 months and all
three scenarios:
...
.small[
1. <b>Paths.</b> Evaluate the AI paths at <i>t</i> and <i>t</i>+1: a lookup, not a solve
   (<i>m</i><sub>t</sub>, <i>d</i><sub>t</sub>, <i>a</i><sub>t</sub>; &psi; and &rho; are constant
   in the three scenarios, and <i>m</i>, <i>d</i> enter only as <i>m</i><sub>t</sub><i>d</i><sub>t</sub>).
...
4. <b>Gaps and the AI-sensitive wage.</b> Form the <b>overhang</b> (<i>G</i><sub>o,t</sub>), the
   <b>shortfall</b> (<i>B</i><sub>o,t</sub>) and the attached force (<i>N</i><sub>C,t</sub>); move the
   actual wage (<i>w</i><sub>C,t</sub>/<i>w</i><sub>t</sub>) a fraction 1&minus;&xi;<sub>m</sub>
   toward the wage that would clear it (<i>w</i><sup>c</sup><sub>C,t</sub>).
5. <b>Separations and openings.</b> At that wage read off labor demand
   ...
   (<i>v</i><sub>C,t</sub>, <i>v</i><sub>N,t</sub>) to replace quits plus &theta;<sub>H</sub> of the
   shortfall (<i>Z</i><sub>t</sub>, <i>B</i><sub>N,t</sub>), over the normal filling rate
   (&pi;&#772;<sub>o</sub>).
...
need a root-find, and both are one-dimensional and monotone.
]
...
tibble(
  ...
  value  = c("3.8%", "0.63% / 1.40%", "0.23", "0.66 / 0.64", "0.76", "1 in 7")
) |>
...
numbers using none of them as inputs. The Python reimplementation matches all six.
]
...
<b>3.</b> Residual of (18), &Delta;ln K<sub>t</sub> &minus; &epsilon;&thinsp;&Delta;ln r<sub>t</sub>,
falls in &Delta;ln r<sub>t</sub>: bisect, back to 2<br>
...
$$s_{L,t0}\Lambda_{C,t} e^{(1-\sigma)\Delta \ln w_{C,t}} + s_{N,t0} e^{(1-\sigma)\Delta \ln w_{N,t}} + B_t e^{(1-\sigma)\Delta \ln r_t} = 1$$

At the end of each month, employment is a *state variable* handed over by the flow
block (<a href="#eqstep-3" class="reflink">step 7, <b>Stocks</b></a>: equation
<a href="../repro.html#eq-36" class="reflink">(36)</a> for employment,
<a href="../repro.html#eq-37" class="reflink">(37)</a> for the unemployment pools),
not a choice. So the demand system is **inverted**: find the prices at which
the realized $(\ell_C, \ell_N)$ is exactly what firms want, and GDP follows.
...
# How the Model Is Actually Solved

.pull-left[
**Two root-finds, both easy**

1. The rental rate: capital demanded equals capital supplied. Demand is strictly
   decreasing in $\Delta \ln r$, so exactly one root; bisection.
2. The actual economy at realized employment: substitute out the capital row and one
   monotone equation in $\Delta \ln r$ remains.
]
...
# Part 3: Where the Equations Come From
...
$$Y_t = \Big[ \sum_i \omega_i^{1/\sigma} y_{i,t}^{(\sigma-1)/\sigma} \Big]^{\sigma/(\sigma-1)}, \qquad y_{i,t} = A_t \alpha_{L,i,t} \ell_{i,t} + \alpha_{K,i,t} k_{i,t}$$
...
$$\psi_t = \text{share of AI-performed instances that are automated}$$
...
$$s_{i,t} = \omega_i \Big(\frac{c_{i,t}}{P_t}\Big)^{1-\sigma}, \qquad P_t \equiv 1$$
...
$$\Delta \ln \mathrm{TFP}_t \approx \sum_i \omega_i d_{i,t} a_{i,t} = s_{L,t0}\, m_t d_t a_t$$
...
$$\Delta \ln s_{L,t} \approx -\underbrace{(1-\rho)\psi_t m_t d_t}_{\text{displacement}} + \underbrace{(1-\sigma)\psi_t m_t d_t a_t}_{\text{weak link}} - \underbrace{(1-\sigma)\tfrac{s_{K,t0}}{s_{L,t0}}\Delta \ln r_t}_{\text{dearer capital}}$$
...
$$\tilde \ell_{N,t} = \Delta \ln (Y_t/L_t) - \sigma \Delta \ln w_t$$

Nursing and electrical work are unaffected, so demand for that labor is an ordinary
...
$$\frac{\ell_{C,t0} - \ell^*_{C,t}}{\ell_{C,t0}} = \frac{s_{N,t0}}{s_{C,t0}} \cdot \frac{\ell^*_{N,t} - \ell_{N,t0}}{\ell_{N,t0}}, \qquad \frac{s_N}{s_C} = 0.60$$
...
$$s_{L,t} = 1 - \big[ s_{K,t0} + s_{L,t0} \psi_t m_t d_t ( e^{-(1-\sigma)a_t} - \rho ) \big] e^{(1-\sigma)\Delta \ln r_t}$$

$$\tilde \ell_{N,t} = -\ln \big( 1 - m_t d_t [ 1 - \rho \psi_t - (1-\psi_t) e^{-(1-\sigma)a_t} ] \big)$$

$$\Delta \ln w_t = \frac{\Delta \ln s_{L,t} + \tilde \ell_{N,t}}{1-\sigma} + \Delta \ln A_t, \qquad \varepsilon \Delta \ln r_t = \Delta \ln K_t$$
...
$$\dot A_t = \nu R_t^{\lambda} A_t^{\phi_R}, \qquad R_t = \iota_{R,t} Y_t, \qquad \Delta \ln R_t = \Delta \ln Y_t$$
...
that is only 1.67 percent a year, so a 28 percent research uplift adds a fraction of a
percentage point, not 28.
...
Immediate, and the whole story over five years.
]
...
Long-run target is ~10 percent, but $\gamma = 0.35$ and cumulation takes decades.
]

.highlight-box[
Over a five-year horizon the innovation block is nearly a rounding error. Over fifty
...
$$D_{C,t} = \max\{0, E_t - q_{C,t}\ell_{C,t}\}, \qquad E_t = \max\{0, \ell_{C,t} - \ell^d_{C,t}\}$$
...
$$H_{j,t} = \chi \frac{S_{j,t} v_{j,t}}{(S_{j,t}^{\iota} + v_{j,t}^{\iota})^{1/\iota}}, \qquad S_{C,t} = U_{C,t} + \mu U_{N,t}$$
...
which matters here because the extreme scenario pushes the ratios far from normal.

The cross-group discount $\mu$ carries all the occupational human capital in the
model: a laid-off software engineer is not instantly a nurse.
...
# Part 4: Results, and What to Trust

---

# Modest: AI as a Normal Technology

```{r modest-fig, echo=FALSE, fig.height=3.2}
p <- d |> filter(scenario == "modest", t >= 2025) |>
  select(t, GDP = gdp, `Average wage` = wage, `AI-sensitive wage` = wage_C) |>
  pivot_longer(-t) |>
  ggplot(aes(t, value, color = name)) +
  geom_line(linewidth = 1.1) +
  scale_color_manual(values = c("GDP" = "#2a78d6", "Average wage" = "#eb6834",
                                "AI-sensitive wage" = "#1baf7a"), name = NULL) +
  labs(x = NULL, y = "percent above the no-AI path") +
  theme(legend.position = "top")
p
```

4 percent of tasks touched. GDP +1.6 percent by 2030, an extra ten months of ordinary
growth. Unemployment 3.9 against a normal 3.8. **Nothing here would be visible in the
macro data.**

---

# Substantial: A Pivotal Technology

.pull-left[
- 12 percent of task instances done by AI
- GDP **+8.3 percent**, growth 5.4 percent a year
- Average wage +2.1 percent, but the **AI-sensitive wage −0.3**
- Labor share 60 → 56
- AI-sensitive unemployment 2.9 → 4.5
]

.pull-right[
```{r subst-fig, echo=FALSE, fig.height=3.4, fig.width=5}
d |> filter(scenario == "substantial", t >= 2025) |>
  select(t, `All other` = wage_N, `AI-sensitive` = wage_C) |>
  pivot_longer(-t) |>
  ggplot(aes(t, value, color = name)) +
  geom_line(linewidth = 1.1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey60") +
  scale_color_manual(values = c("All other" = "#2a78d6", "AI-sensitive" = "#eb6834"),
                     name = NULL) +
  labs(x = NULL, y = "wage, pct above no-AI") +
  theme(legend.position = "top")
```
]

A four-point fall in the labor share in four years is roughly the entire US decline of
the four decades after 1980.

---

# Extreme: Transformative

```{r extreme-fig, echo=FALSE, fig.height=3.3}
d |> filter(scenario == "extreme", t >= 2025) |>
  select(t, GDP = gdp, `AI-sensitive wage` = wage_C, `All-other wage` = wage_N,
         `AI-sensitive unemployment` = u_C) |>
  pivot_longer(-t) |>
  ggplot(aes(t, value, color = name)) +
  geom_line(linewidth = 1.1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey70") +
  scale_color_manual(values = c("GDP" = "#2a78d6", "AI-sensitive wage" = "#eb6834",
                                "All-other wage" = "#1baf7a",
                                "AI-sensitive unemployment" = "#4a3aa7"), name = NULL) +
  labs(x = NULL, y = "percent") +
  theme(legend.position = "top")
```

GDP +32 percent and growth at 15 percent a year, with the AI-sensitive wage 11 percent
*below* its no-AI path and nearly one AI-sensitive worker in five unemployed.
...
| $\varepsilon$ | 1 | 3 | 6 | $\infty$ |
|---|---|---|---|---|
| wage | −1.6 | +2.1 | +3.7 | +5.6 |
...
| $\xi$ | 0 | 0.5 | 0.9 |
|---|---|---|---|
| AI-sensitive wage | −42 | −11.5 | +2.8 |
| AI-sensitive unemp. | 2.6 | 17.9 | 24.0 |
...
# Extension: Where Would "AI Slop" Show Up?
...
in $\rho$, and it is the only case here that cushions wages (+0.7) and raises the
labor share (57.3) — at the cost of lower output. "Slop that makes work" and "slop
...
|---|---|---|
| $m_{2030}$ | 0.44 | 0.2 / 0.3 / 0.5 |
| $d_{2030}$ | 0.40 | 0.2 / 0.4 / 0.6 |
...
tibble(
  ...
  Cells = c(80, 40, 49),
  `Largest deviation` = c(0.08, 0.05, 0.13)
) |>
  knitr::kable(format = "html")
```

Also matched without being targeted: the steady-state switching share of 1 in 7,
filling rates 0.66 / 0.64, $\chi = 0.76$, the logistic slopes $\kappa_m = 0.33$ and
$\kappa_d = 0.51$, and the worked example's exact TFP gain of 0.029.

.highlight-box[
One genuine discrepancy: the all-workers unemployment rate is 0.07 points low
everywhere, because Table 1 rounds the normal pool to 0.038 while the paper's own
derivation uses 0.0384. At 0.0384 the row lands exactly.
]
...
2. **TFP and distribution are separate questions.** $\psi$ moves the labor share
   without moving productivity at all.
3. **$\sigma < 1$ is load-bearing.** Set it above one and the labor-share results
   change sign.
4. **Capital supply decides whether workers gain**, and wage rigidity decides whether
   the cost is paid in wages or in jobs.
5. **The growth channel is small over five years**, by construction.

.blue-box[
The reproduction: `aiscen` Python package, 98 tests, reproduces Tables 3, 5 and 6 to
the printed precision. Narrative walkthrough with every equation in `repro.qmd`.
]

---
name: three-labels

# Postscript: Three Labels, Seven Inputs
...
effect and the interactions, instead of three labelled points.
]
...
Holding the three size inputs at extreme, the four friction inputs alone move AI-sensitive
...
The link is a **private** Claude artifact: it opens only for accounts it has been shared with, so
share it from the page's share menu before circulating this deck.
...
do. Grid caveat: $a_t$ is built from the common mid-2026 anchor, as the explorer does for
off-scenario values, so the grid's corners reproduce the named scenarios to within 0.03 points.
]
...
**one scalar equation** to bisect with `bisect()`, `numerics.py:4`; the monthly rental rate uses
the same routine via `expand_and_bisect()`, `statics.py:92`. The definition row is not one of the ten.
...
| $\bar S_C = \bar U_C + \bar\mu \bar U_N$, $\ \bar S_N = \bar\mu \bar U_C + \bar U_N$ | — | **definition** (33), not a condition; substituted in steps 2 and 3 |
| $\bar f_C = \bar H_C/\bar S_C + \bar\mu \bar H_N/\bar S_N$ | 1 | step 2, substituted to express $\bar f_C$ |
| $\bar f_o \bar U_o = \bar H_o$, for $o = C$ | 1 | **step 2**, bisected at `steady.py:64` |
...
| mean filling $=$ mean $\bar\pi$ | 1 | **step 3**, bisected at `steady.py:84` |
]
...
The residual runs from $-\infty$ at $\bar U_C \to 0$ to $+0.086$ at $\bar U_C = \bar U$, so bisect on
...
(`steady.py:50-90`). Below, <span style="color:#228B22;font-weight:600">green</span> is
...
row is the only *condition*, and it is the one bisected (`fill_resid`, `steady.py:78`,
bisected at `:84`). How wrong a guess is = the mean filling rate it implies, minus the
...
.highlight-box[
.small[
**Substitute the explicit into the implicit, not the reverse.** Putting both definitions into
the condition turns three unknowns into one scalar equation in $\chi$. Going the other way,
using the condition to write $\bar\pi_C$ in terms of $\bar\pi_N$, is equally valid and gives the
same answer, but it leaves a $2 \times 2$ system in $\chi$ and $\bar\pi_N$: the $N$ definition
still has to be substituted before it reduces to that same scalar equation. Doing it in the
first order is what keeps every root-find in this model one-dimensional and monotone.
]
]
...
# Backup: Month 0 &mdash; Steps 2 and 3
...
| Eq. | Produces | Month 0 |
|---|---|---|
| 18, 6 | $\Delta \ln r^*$, the bisection root | 0.000776 |
| 15 | $\tilde \ell_{N,t}$ | 0.001246 |
| 14 | $s_{L,t}^*$ | 0.59926 |
| 16 | $\Delta \ln w^*$ | $2.83 \cdot 10^{-5}$ |
| 5 | $\Delta \ln (Y/L)^*$ | 0.001260 |
| 17 | $\Delta \ln K^*$ | 0.002329 |
| 45 | $\Delta \ln \mathrm{TFP}$ | 0.000328 |
| 13 | $\ell^*_{N,t}$ | 0.36216 |
| 13 | $\ell^*_{C,t}$ | 0.59984 |
]
...
# Backup: Month 0 &mdash; Steps 4 and 5
...
# Backup: Month 0 &mdash; Steps 6 and 7
...
# Backup: Month 0 &mdash; Steps 8 and 9

.small[
Report the economy at **realized** employment, then feed the GDP gap into ideas.

| Eq. | Produces | Month 0 |
|---|---|---|
| 39 | $\Delta \ln Y$ | 0.001260 |
| 39 | $\Delta \ln w_N$, $\Delta \ln r$, $\Delta \ln K$ | 0.002519, 0.000776, 0.002329 |
| 39 | $s_{L,t}$ | 0.59926 |
| 19 | $\tilde \ell_{C,t}$ | $-0.000752$ |
| --- | $u^x$, $X$, $G$ | 0, 0.000461, 0.000482 |
| 22 | $\Delta \ln R = \Delta \ln Y$ | 0.001260 |
| 42 | $\Delta g$ | $2.105 \cdot 10^{-5}$ |
| 43 | $\Delta \ln A_{t+1}$ | $1.754 \cdot 10^{-6}$ |
]

.highlight-box[
.small[
The GDP gap splits as TFP plus capital deepening: $0.000328 + 0.40 \times 0.002329 = 0.00126$, so **three quarters of it is capital**, a direct consequence of $\varepsilon = 3$. Unemployment is still exactly normal, $u^x = 0$, and the ideas stock leaves month 0 at $1.754 \cdot 10^{-6}$.
]
]
...
$t_0$ and deliberately outside the 44: `steady.py:51`.
]
]
...
the squeeze reaches **0.004 / 0.047 / 0.271** log points at 2030 against gains of
...
Equation (19), split into its two terms:

$$\tilde \ell_{N,t} \approx \underbrace{(1-\rho)\psi_t m_t d_t}_{\text{no } a_t \text{ in it}} + \underbrace{(1-\sigma)(1-\psi_t) m_t d_t a_t}_{\text{proportional to } a_t}$$
...
and then 7.58, straight through the calibrated $\varepsilon = 3$.

.small[
What the model cannot host: quality itself. Instances are homogeneous and prices are
quality-adjusted by assumption, so slop can only be a smaller cost saving, never the
same cost for a worse product. And if errors propagate downstream, Hulten's theorem
(Equation 9) no longer applies, because it assumes an efficient economy.
]
...
# Backup: The One Number That Does Not Reproduce
...
At $\bar U = 0.038$ the reimplementation gives an all-workers unemployment rate 0.07
points below every published figure (4.53 against 4.6 in the substantial scenario). At
$\bar U = 0.0384$ every affected cell lands exactly, and nothing else moves.
...
AI adoption begins in mid-2026, so the paths have already separated by `r start_pc`% before 2027 opens.
]
